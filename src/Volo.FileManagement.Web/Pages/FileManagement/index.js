$(function () {
    ...
    var DOWNLOAD_ENDPOINT = '/api/file-management/file-descriptor/download/{fileId}?token={token}';
    ...
    var directoryRenameModal = new abp.ModalManager({
        ...
        scriptUrl: abp.appPath + "Pages/FileManagement/Directory/changeNameModal.js",
    ...
    var directoryCreateModal = new abp.ModalManager({
        ...
        scriptUrl: abp.appPath + "Pages/FileManagement/Directory/createModal.js",
        modalClass: "directoryCreateModal"
    });

    var directoryMoveModal = new abp.ModalManager({
        viewUrl: abp.appPath + "FileManagement/Directory/MoveModal",
        scriptUrl: abp.appPath + "Pages/FileManagement/Directory/moveModal.js",
        modalClass: "directoryMoveModal"
    });

    var fileRenameModal = new abp.ModalManager({
        viewUrl: abp.appPath + "FileManagement/File/ChangeNameModal",
        scriptUrl: abp.appPath + "Pages/FileManagement/File/changeNameModal.js",
        modalClass: "fileChangeNameModal"
    });

    var fileMoveModal = new abp.ModalManager({
        viewUrl: abp.appPath + "FileManagement/File/MoveModal",
        scriptUrl: abp.appPath + "Pages/FileManagement/File/moveModal.js",
    ...
    var fileManagementPage = {
        ...
        directoryId: {
            ...
            set: function (id, byTree) {
                ...
                if(byTree){
                    fileManagementPage.dataTable.ajax.reloadEx();
                }else{
                    fileManagementPage.dataTable.ajax.reloadEx();
        ...
        tree: {
            ...
            highlightNode: function(nodeId){
                $('#DirectoryTree li').removeClass('jstree-wholerow-clicked');
                $('li[id="' + nodeId + '"]').addClass('jstree-wholerow-clicked');
            },

            generateTextOnTree: function (directory) {
                var itemClass = ' pg-text-no-members';

                return '<span title="' + directory.name + '" class="pg-text' + itemClass + '" data-pg-id="' + directory.id + '">' + directory.name + '</span>';
            },
            ...
            on_move: function (e, data) {
                ...
                var byTree = fileManagementPage.moveDirectoryId === undefined;

                if(!byTree){
                    fileManagementPage.moveDirectoryId = undefined;
                    return;
                }
                        ...
                        'items': function ($node){
                            ...
                            var contextMenu = {
                                createFolder: {
                                    ...
                                    'action': function (){
                                        directoryCreateModal.open({ parentId: $node.id });
                                    }
                                },
                                rename: {
                                    'label': l('Rename'),
                                    'action': function (){
                                        directoryRenameModal.open({ id: $node.id });
                                    }
                                },
                                move: {
                                    'label': l('Move'),
                                    'action': function(){
                                        fileManagementPage.moveDirectoryId = $node.id;
                                        directoryMoveModal.open({ id: $node.id });
                                    }
                                },
                                delete: {
                                    'label': l('Delete'),
                                    'action': function (){
                                        abp.message.confirm(l("DirectoryDeleteConfirmationMessage"), function (confirm){
                                            if(!confirm){
                                                return;
                                            }

                                            directoryDescriptorService.delete($node.id)
                                                .then(function () {
                                                    abp.notify.success(l('DeletedSuccessfully'));
                                                    fileManagementPage.directoryId.set(fileManagementPage.tree.getNormalizedId($node.parent));
                                                });
                                        });
                                    }
                                }
                            ...
                            if(!fileManagementPermissions.Directory_Update){
                                delete contextMenu.rename;
                                delete contextMenu.move;
                            }

                            if(!fileManagementPermissions.Directory_Delete){
                                delete contextMenu.delete;
                            }

                            if(!fileManagementPermissions.Directory_Create){
                                delete contextMenu.createFolder;
                            }
        ...
        route: {
            ...
            reDraw: function(selectedNodeId){
                ...
                this.$list.append('<li class="breadcrumb-item" directory-id=""><i class="fa fa-home"></i><span>' + l('AllFiles') + '</span></li>');
        ...
        downloadFile: function(fileId){
            volo.fileManagement.files.fileDescriptor.getDownloadToken(fileId).then(function(result){

                var url = DOWNLOAD_ENDPOINT.replace('{fileId}', fileId).replace('{token}', result.token);
        ...
        initUppy: function(){
            fileManagementPage.uppy.uppy = new Uppy.Uppy({
        ...
        initDataTable: function(){
                ...
                columnDefs: [
                            ...
                            items:
                                [
                                    ...
                                    },
                                    {
                                        ...
                                        action: function (data) {
                                            if(data.record.isDirectory){
                                                fileManagementPage.moveDirectoryId = data.record.id;
                                                directoryMoveModal.open({ id: data.record.id });
                                            }else{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row){
                            ...
                            if(row.isDirectory){
                                return '<span class="directoryName" directory-name="' + data + '" directory-id="' + row.id + '"><i class="directory-content-icon fa fa-folder text-primary text-center me-1"></i>' + data + '</span>';
                            }else{
                                var fileIcon = "";
                                if(row.iconInfo.type === 0){
                                    // font-awesome
                                    fileIcon = '<i class="directory-content-icon ' + row.iconInfo.icon + ' text-muted text-center me-1"></i>';
                                }else{
                                    // url
                                    fileIcon = '<span class="directory-content-icon me-1" style="background-image: url(' + row.iconInfo.icon + ')"></span>';
                                }
            ...
            configuration.language.infoEmpty = l('Datatable:InfoEmpty');
            configuration.language.info = l('Datatable:Info');
        ...
        initListeners: function(){
            ...
            var modalListeners = function(){
                directoryRenameModal.onResult(function () {
                    ...
                    directoryDescriptorService.get(directoryId).then(function (result){
                        var node = fileManagementPage.tree.getNode(directoryId);
                        node.text = result.name;
                        node.original.name = result.name;
                        fileManagementPage.tree.selectNode(directoryId);
                        fileManagementPage.directoryId.set(directoryId, true);
                    });
                });
                ...
                directoryMoveModal.onResult(function () {
                    directoryDescriptorService.get(fileManagementPage.moveDirectoryId).then(function(result){
                        fileManagementPage.tree.moveNode(result.id, result.parentId ? result.parentId : 'root-node')
                        fileManagementPage.dataTable.ajax.reloadEx();
                    });
                });
            }
            ...
            var uppyListeners = function(){
                ...
                fileManagementPage.uppy.uppy.on('dashboard:modal-closed', function () {
                    fileManagementPage.dataTable.ajax.reloadEx();
                    fileManagementPage.uppy.uppy.cancelAll();
                });
