...
public class ResponseAppService : FormsAppServiceBase, IResponseAppService
{
    ...
    {
        var formResponse = await GetResponseWithoutTenantFilterAsync(id);
    ...
    public virtual async Task<FormDto> GetFormDetailsAsync(Guid formId)
    {
        ...
        if (form.RequiresLogin && form.TenantId != CurrentTenant.Id)
        {
            return null;
        }
    ...
    public virtual async Task<FormResponseDto> SaveAnswersAsync(Guid formId, CreateResponseDto input)
    {
        ...
        if (form.RequiresLogin)
        {
            ...
            if (form.HasLimitOneResponsePerUser &&
                await ResponseRepository.UserResponseExistsAsync(formId, CurrentUser.GetId()))
            {
        ...
        var newResponse = new FormResponse(GuidGenerator.Create(), form.Id, CurrentUser.Id, input.Email, CurrentTenant.Id);

        foreach (var answerDto in input.Answers)
        {
            newResponse.AddOrUpdateAnswer(
                questionId: answerDto.QuestionId,
                answerId: GuidGenerator.Create(),
    ...
    {
        var form = await GetFormWithoutTenantFilterAsync(input.FormId);
        ...
        {
            formResponse.AddOrUpdateAnswer(
    ...
    private Task<FormResponse> GetResponseWithoutTenantFilterAsync(Guid id)
    {
        using (DataFilter.Disable<IMultiTenant>())
        {
            return ResponseRepository.GetAsync(id);
        }
    }
}