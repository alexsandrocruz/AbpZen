...
public class ExternalLocalizationStoreCache : IExternalLocalizationStoreCache, ISingletonDependency
{
    ...
    public ExternalLocalizationStoreCache(IDistributedCache<LocalizationResourceRecordCacheItem> resourceCache, IDistributedCache<AllLocalizationResourcesCacheItem> allResourcesCache)
    {
    ...
    {
        return await AllResourcesCache.GetOrAddAsync(
            AllLocalizationResourcesCacheItem.CacheKey,
            factory
        );
    }
    ...
    public virtual async Task InvalidateAsync(IEnumerable<string> changedResourceNames)
    {
        ...
        foreach (var resourceName in changedResourceNames)
        {
            await ResourceCache.RemoveAsync(resourceName);
        }
    }
    
    public virtual LocalizationResourceRecordCacheItem CreateResourceCacheItem(LocalizationResourceRecord? resourceRecord)
    {
