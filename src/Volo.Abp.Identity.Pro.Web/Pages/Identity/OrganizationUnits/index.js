...
(function () {
    ...
    $(function () {
        ...
        var _deleteModal = new abp.ModalManager(abp.appPath + 'Identity/OrganizationUnits/DeleteModal');
        _deleteModal.onOpen(function () {
            var $form = _deleteModal.getForm();
            $form.find('#assign').click(function () {
                $form.find('#OrganizationUnit_AssignToOrganizationUnitId').show();
                $form.find('[type=submit]').attr("disabled","disabled")
            })
            $form.find('#unassign').click(function () {
                $form.find('#OrganizationUnit_AssignToOrganizationUnitId').hide();
                $form.find('#OrganizationUnit_AssignToOrganizationUnitId').val("");
                $form.find('[type=submit]').removeAttr("disabled");
            })

            $("#OrganizationUnit_AssignToOrganizationUnitId").on("change", function () {
                var val = $(this).val();
                if(val === ''){
                    $form.find('[type=submit]').attr("disabled","disabled")
                }else{
        ...
        var _moveAllUsersModal = new abp.ModalManager(abp.appPath + 'Identity/OrganizationUnits/MoveAllUsersModal');
        ...
        var organizationTree = {
            ...
            selectedOu: {
                ...
                set: function (ouInTree) {
                    ...
                    if(ouInTree){
                        $(".selected-organizationunit-name").html(ouInTree.original.displayName);
                    }
                    members.load();
                    roles.load();
                }
            ...
            contextMenu: function (node) {
                var items = {
                    ...
                    'MoveAllUsers': {
                         label: l('MoveAllUsers'),
                         icon: 'la la-move',
                         _disabled: !_permissions.manageMembers,
                         action: function () {
                             _organizationUnitService.get(node.id).done(function (organizationUnit) {
                                 if(organizationUnit.userCount === 0){
                                     abp.message.warn(l('ThereIsNoUsersCurrentlyInThisOrganizationUnit'));
                                     return;
                                 }
            ...
            init: function () {
                organizationTree.getTreeDataFromServer(function (treeData) {
                    ...
                    _deleteModal.onResult(function (ou) {
                        abp.notify.success(l('DeletedSuccessfully'));
                        organizationTree.reload();
                    });

                    _moveAllUsersModal.onResult(function (ou) {
                        abp.notify.success(l('SavedSuccessfully'));
                        organizationTree.reload();
                    });
        ...
        var members = {
            ...
            openAddModal: function () {
                var ouId = organizationTree.selectedOu.id;
                var displayName = organizationTree.selectedOu.displayName;
                if (!ouId) {
                    return;
                }
                _addMemberModal.open({
                    ...
                    OrganizationUnitName :displayName
            ...
            init: function () {
                ...
                members.hideTable();
                $("#Members-tab").on('shown.bs.tab', function () {
                    if (organizationTree.selectedOu.id && members.dataTable) {
                        members.dataTable.ajax.reloadEx();
                    }
                })
            }
        };
        ...
        var roles = {
            ...
            init: function () {
                ...
                $("#Roles-tab").on('shown.bs.tab', function () {
                    if (organizationTree.selectedOu.id && roles.dataTable) {
                        roles.dataTable.ajax.reloadEx();
                    }
                })
            }
        };
    ...
    let selectAllHeaderTitle = '<div class="form-check no-height">' +
        '<input type="checkbox" id="select_all" name="select-all" class="form-check-input custom-selectbox" value="false" data-val="false">' +
        '<label class="form-check-label" for="select_all"></label></div>';
    ...
    const uncheckAndRemoveAll = function () {
        ...
        var unselectedItems = $(`input[name="${selectedCheckboxNames}"]:not(:checked)`);
    ...
    function checkSelectedItems() {
        ...
        $(`input[name="${selectedCheckboxNames}"]`).each(function () {
