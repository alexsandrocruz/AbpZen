...
export const DEFAULT_SECURITY_LOGS_ENTITY_PROPS = EntityProp.createMany<IdentitySecurityLogDto>([
  {
    ...
    valueResolver: data => {
      ...
      return of(
        formatDate(data.record.creationTime, getShortDateShortTimeFormat(config), localeId),
      );
    },
  },
  {
  ...
  },
  {
    ...
    valueResolver: data => {
      return of(
        `<div style="max-width: 135px" class="pointer abp-ellipsis-inline">${
