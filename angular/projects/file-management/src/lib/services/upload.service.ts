...
import XHRUpload, { Headers } from '@uppy/xhr-upload';
import {
  ApiInterceptor,
  EnvironmentService,
  LocalizationService,
...
export class UploadService implements OnDestroy {
  ...
  getPreInfo = (files: ChosenFile[]) => {
    return this.callPreInfoWithFiles(files).pipe(
  ...
  initUppy(trigger, opts?: Partial<UppyOptions<any>>) {
    const headers: Headers = Object.assign(<Headers>{}, this.apiInterceptor.getAdditionalHeaders());
    headers[this.headerName] = this.httpXsrfToken.getToken();

    this.subscriptionService.addOne(
      this.localization.getResource$('FileManagement').pipe(take(1)),
      resource => {
    ...
    this.subscriptionService.addOne(
      fromEvent(this.uppy, 'file-added').pipe(
        map(files => files[0]),
        bufferTime(500),
        filter(args => args.length > 0),
        switchMap(files => this.getPreInfo(files)),
      ),
    );

    this.subscriptionService.addOne(fromEvent(this.uppy, 'complete'), () => {
      this.cancelModal();
  ...
  private updateXHREndpoint() {
    const directoryId = this.updateStream.currentDirectory;
  ...
  private callPreInfoWithFiles(files) {
    const currentId = this.updateStream.currentDirectory;
