...
namespace Sapienza.FabioRibeiro.MauiBlazor.Navigation;

public class FabioRibeiroMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public FabioRibeiroMenuContributor(IConfiguration configuration)
    {
    ...
    {
        var l = context.GetLocalizer<FabioRibeiroResource>();

        context.Menu.Items.Insert(
            0,
            new ApplicationMenuItem(
                FabioRibeiroMenus.Home,
                "Home",
                "/",
                icon: "fas fa-home",
                order: 1
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                FabioRibeiroMenus.HostDashboard,
                l["Menu:Dashboard"],
                "/HostDashboard",
                icon: "fa fa-chart-line",
                order: 2
            ).RequirePermissions(FabioRibeiroPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                FabioRibeiroMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "/Dashboard",
                icon: "fa fa-chart-line",
                order: 2
            ).RequirePermissions(FabioRibeiroPermissions.Dashboard.Tenant)
        );
