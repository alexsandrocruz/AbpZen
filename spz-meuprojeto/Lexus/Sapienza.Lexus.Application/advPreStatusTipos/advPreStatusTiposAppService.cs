    ...
    IadvPreStatusTiposAppService
{
    ...
    private readonly IRepository<Sapienza.Lexus.advPreStatus.advPreStatus, Guid> _advPreStatusRepository;

    public advPreStatusTiposAppService(
        IRepository<Sapienza.Lexus.advPreStatusTipos.advPreStatusTipos, Guid> repository,
        IRepository<Sapienza.Lexus.advPreStatus.advPreStatus, Guid> advPreStatusRepository
    )
    {
        _repository = repository;
        _advPreStatusRepository = advPreStatusRepository;
    }
    ...
    public virtual async Task<advPreStatusTiposDto> GetAsync(Guid id)
    {
        ...
        var advPreStatus = await _advPreStatusRepository.FindAsync(entity.advPreStatusId);
        dto.advPreStatusDisplayName = advPreStatus?.titulo;
    ...
    public virtual async Task<PagedResultDto<advPreStatusTiposDto>> GetListAsync(advPreStatusTiposGetListInput input)
    {
        ...
        var advPreStatusIds = entities
            .Select(x => x.advPreStatusId)
            .Distinct()
            .ToList();

        if (advPreStatusIds.Any())
        {
            var parents = await _advPreStatusRepository.GetListAsync(x => advPreStatusIds.Contains(x.Id));
            var parentMap = parents.ToDictionary(x => x.Id, x => x.titulo);

            foreach (var dto in dtoList)
            {
                if (parentMap.TryGetValue(dto.advPreStatusId, out var displayName))
                {
                    dto.advPreStatusDisplayName = displayName;
                }
            }
        }
    ...
    protected virtual IQueryable<Sapienza.Lexus.advPreStatusTipos.advPreStatusTipos> ApplyFilters(IQueryable<Sapienza.Lexus.advPreStatusTipos.advPreStatusTipos> queryable, advPreStatusTiposGetListInput input)
    {
        return queryable
            ...
            // ========== FK Filters ==========
            .WhereIf(input.advPreStatusId != null, x => x.advPreStatusId == input.advPreStatusId)
            ;
    }
}
