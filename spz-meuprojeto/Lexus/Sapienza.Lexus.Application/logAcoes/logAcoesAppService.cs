    ...
    IlogAcoesAppService
{
    ...
    private readonly IRepository<Sapienza.Lexus.logCampos.logCampos, Guid> _logCamposRepository;

    public logAcoesAppService(
        IRepository<Sapienza.Lexus.logAcoes.logAcoes, Guid> repository,
        IRepository<Sapienza.Lexus.logCampos.logCampos, Guid> logCamposRepository
    )
    {
        _repository = repository;
        _logCamposRepository = logCamposRepository;
    }
    ...
    public virtual async Task<logAcoesDto> GetAsync(Guid id)
    {
        ...
        if (entity.logCamposId != null)
        {
            var parent = await _logCamposRepository.FindAsync(entity.logCamposId.Value);
            dto.logCamposDisplayName = parent?.campo;
        }
    ...
    public virtual async Task<PagedResultDto<logAcoesDto>> GetListAsync(logAcoesGetListInput input)
    {
        ...
        var logCamposIds = entities
            .Where(x => x.logCamposId != null)
            .Select(x => x.logCamposId.Value)
            .Distinct()
            .ToList();

        if (logCamposIds.Any())
        {
            var parents = await _logCamposRepository.GetListAsync(x => logCamposIds.Contains(x.Id));
            var parentMap = parents.ToDictionary(x => x.Id, x => x.campo);

            foreach (var dto in dtoList.Where(x => x.logCamposId != null))
            {
                if (parentMap.TryGetValue(dto.logCamposId.Value, out var displayName))
                {
                    dto.logCamposDisplayName = displayName;
                }
            }
        }
    ...
    protected virtual IQueryable<Sapienza.Lexus.logAcoes.logAcoes> ApplyFilters(IQueryable<Sapienza.Lexus.logAcoes.logAcoes> queryable, logAcoesGetListInput input)
    {
        return queryable
            ...
            // ========== FK Filters ==========
            .WhereIf(input.logCamposId != null, x => x.logCamposId == input.logCamposId)
            ;
    }
}
