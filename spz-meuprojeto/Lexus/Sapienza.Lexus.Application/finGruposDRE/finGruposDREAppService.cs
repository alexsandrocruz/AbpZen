    ...
    IfinGruposDREAppService
{
    ...
    private readonly IRepository<Sapienza.Lexus.finPlanoContasGrupos.finPlanoContasGrupos, Guid> _finPlanoContasGruposRepository;

    public finGruposDREAppService(
        IRepository<Sapienza.Lexus.finGruposDRE.finGruposDRE, Guid> repository,
        IRepository<Sapienza.Lexus.finPlanoContasGrupos.finPlanoContasGrupos, Guid> finPlanoContasGruposRepository
    )
    {
        _repository = repository;
        _finPlanoContasGruposRepository = finPlanoContasGruposRepository;
    }
    ...
    public virtual async Task<finGruposDREDto> GetAsync(Guid id)
    {
        ...
        var finPlanoContasGrupos = await _finPlanoContasGruposRepository.FindAsync(entity.finPlanoContasGruposId);
        dto.finPlanoContasGruposDisplayName = finPlanoContasGrupos?.titulo;
    ...
    public virtual async Task<PagedResultDto<finGruposDREDto>> GetListAsync(finGruposDREGetListInput input)
    {
        ...
        var finPlanoContasGruposIds = entities
            .Select(x => x.finPlanoContasGruposId)
            .Distinct()
            .ToList();

        if (finPlanoContasGruposIds.Any())
        {
            var parents = await _finPlanoContasGruposRepository.GetListAsync(x => finPlanoContasGruposIds.Contains(x.Id));
            var parentMap = parents.ToDictionary(x => x.Id, x => x.titulo);

            foreach (var dto in dtoList)
            {
                if (parentMap.TryGetValue(dto.finPlanoContasGruposId, out var displayName))
                {
                    dto.finPlanoContasGruposDisplayName = displayName;
                }
            }
        }
    ...
    protected virtual IQueryable<Sapienza.Lexus.finGruposDRE.finGruposDRE> ApplyFilters(IQueryable<Sapienza.Lexus.finGruposDRE.finGruposDRE> queryable, finGruposDREGetListInput input)
    {
        return queryable
            ...
            // ========== FK Filters ==========
            .WhereIf(input.finPlanoContasGruposId != null, x => x.finPlanoContasGruposId == input.finPlanoContasGruposId)
            ;
    }
}
