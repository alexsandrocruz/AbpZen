    ...
    IadvPreCheckListsGruposAppService
{
    ...
    private readonly IRepository<Sapienza.Lexus.advPreCheckLists.advPreCheckLists, Guid> _advPreCheckListsRepository;
    ...
    public advPreCheckListsGruposAppService(
        ...
        IRepository<Sapienza.Lexus.advPreCheckLists.advPreCheckLists, Guid> advPreCheckListsRepository
    )
    {
        _repository = repository;
        _advPreCheckListsRepository = advPreCheckListsRepository;
    }
    ...
    public virtual async Task<advPreCheckListsGruposDto> GetAsync(Guid id)
    {
        ...
        if (entity.advPreCheckListsId != null)
        {
            var parent = await _advPreCheckListsRepository.FindAsync(entity.advPreCheckListsId.Value);
            dto.advPreCheckListsDisplayName = parent?.titulo;
        }
    ...
    public virtual async Task<PagedResultDto<advPreCheckListsGruposDto>> GetListAsync(advPreCheckListsGruposGetListInput input)
    {
        ...
        var advPreCheckListsIds = entities
            .Where(x => x.advPreCheckListsId != null)
            .Select(x => x.advPreCheckListsId.Value)
            .Distinct()
            .ToList();

        if (advPreCheckListsIds.Any())
        {
            var parents = await _advPreCheckListsRepository.GetListAsync(x => advPreCheckListsIds.Contains(x.Id));
            var parentMap = parents.ToDictionary(x => x.Id, x => x.titulo);

            foreach (var dto in dtoList.Where(x => x.advPreCheckListsId != null))
            {
                if (parentMap.TryGetValue(dto.advPreCheckListsId.Value, out var displayName))
                {
                    dto.advPreCheckListsDisplayName = displayName;
                }
            }
        }
    ...
    protected virtual IQueryable<Sapienza.Lexus.advPreCheckListsGrupos.advPreCheckListsGrupos> ApplyFilters(IQueryable<Sapienza.Lexus.advPreCheckListsGrupos.advPreCheckListsGrupos> queryable, advPreCheckListsGruposGetListInput input)
    {
        return queryable
            ...
            // ========== FK Filters ==========
            .WhereIf(input.advPreCheckListsId != null, x => x.advPreCheckListsId == input.advPreCheckListsId)
            ;
    }
}
