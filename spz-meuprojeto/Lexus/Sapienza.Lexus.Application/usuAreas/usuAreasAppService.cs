    ...
    IusuAreasAppService
{
    ...
    private readonly IRepository<Sapienza.Lexus.usuCargos.usuCargos, Guid> _usuCargosRepository;

    public usuAreasAppService(
        IRepository<Sapienza.Lexus.usuAreas.usuAreas, Guid> repository,
        IRepository<Sapienza.Lexus.usuCargos.usuCargos, Guid> usuCargosRepository
    )
    {
        _repository = repository;
        _usuCargosRepository = usuCargosRepository;
    }
    ...
    public virtual async Task<usuAreasDto> GetAsync(Guid id)
    {
        ...
        if (entity.usuCargosId != null)
        {
            var parent = await _usuCargosRepository.FindAsync(entity.usuCargosId.Value);
            dto.usuCargosDisplayName = parent?.titulo;
        }
    ...
    public virtual async Task<PagedResultDto<usuAreasDto>> GetListAsync(usuAreasGetListInput input)
    {
        ...
        var usuCargosIds = entities
            .Where(x => x.usuCargosId != null)
            .Select(x => x.usuCargosId.Value)
            .Distinct()
            .ToList();

        if (usuCargosIds.Any())
        {
            var parents = await _usuCargosRepository.GetListAsync(x => usuCargosIds.Contains(x.Id));
            var parentMap = parents.ToDictionary(x => x.Id, x => x.titulo);

            foreach (var dto in dtoList.Where(x => x.usuCargosId != null))
            {
                if (parentMap.TryGetValue(dto.usuCargosId.Value, out var displayName))
                {
                    dto.usuCargosDisplayName = displayName;
                }
            }
        }
    ...
    protected virtual IQueryable<Sapienza.Lexus.usuAreas.usuAreas> ApplyFilters(IQueryable<Sapienza.Lexus.usuAreas.usuAreas> queryable, usuAreasGetListInput input)
    {
        return queryable
            ...
            // ========== FK Filters ==========
            .WhereIf(input.usuCargosId != null, x => x.usuCargosId == input.usuCargosId)
            ;
    }
}
