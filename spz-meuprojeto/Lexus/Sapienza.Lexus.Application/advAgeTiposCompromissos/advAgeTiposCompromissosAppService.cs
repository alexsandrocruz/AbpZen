    ...
    IadvAgeTiposCompromissosAppService
{
    ...
    public virtual async Task<advAgeTiposCompromissosDto> GetAsync(Guid id)
    {
        ...
        if (entity.advCompromissosId != null)
        {
            var parent = await _advCompromissosRepository.FindAsync(entity.advCompromissosId.Value);
            dto.advCompromissosDisplayName = parent?.dataPublicacao;
        }
    ...
    public virtual async Task<PagedResultDto<advAgeTiposCompromissosDto>> GetListAsync(advAgeTiposCompromissosGetListInput input)
    {
        ...
        var advCompromissosIds = entities
            .Where(x => x.advCompromissosId != null)
            .Select(x => x.advCompromissosId.Value)
            .Distinct()
            .ToList();
        ...
        if (advCompromissosIds.Any())
        {
            ...
            foreach (var dto in dtoList.Where(x => x.advCompromissosId != null))
            {
                if (parentMap.TryGetValue(dto.advCompromissosId.Value, out var displayName))
                {
