...
namespace Sapienza.EventoZen.MauiBlazor.Navigation;

public class EventoZenMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public EventoZenMenuContributor(IConfiguration configuration)
    {
    ...
    {
        var l = context.GetLocalizer<EventoZenResource>();
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                EventoZenMenus.HostDashboard,
                l["Menu:Dashboard"],
                "/HostDashboard",
                icon: "fa fa-chart-line",
                order: 2
            ).RequirePermissions(EventoZenPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                EventoZenMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "/Dashboard",
                icon: "fa fa-chart-line",
                order: 2
            ).RequirePermissions(EventoZenPermissions.Dashboard.Tenant)
        );
