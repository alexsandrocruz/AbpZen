...
using Volo.FileManagement.Menus;
...
public class FileManagementWebHostModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(FileManagementWebHostModule).Assembly
            );
        });
    }
    ...
    private void ConfigureMenus(IConfiguration configuration)
    {
        ...
        {
            options.MenuContributors.Add(new FileManagementWebHostMenuContributor(configuration));
        });
    }
    ...
    private void ConfigureAutoMapper()
    {
        ...
        {
            options.AddMaps<FileManagementWebHostModule>();
        });
    }
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<FileManagementDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}Volo.FileManagement.Domain", Path.DirectorySeparatorChar)));
