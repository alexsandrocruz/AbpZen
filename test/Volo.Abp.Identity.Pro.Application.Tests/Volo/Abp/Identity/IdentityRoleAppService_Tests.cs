...
public class IdentityRoleAppService_Tests : AbpIdentityApplicationTestBase
{
    ...
    public async Task UpdateClaimsAsync()
    {
        ...
        var exception = await Assert.ThrowsAsync<UserFriendlyException>(async () => await _roleAppService.UpdateClaimsAsync(moderator.Id, 
        [
            new IdentityRoleClaimDto { ClaimType = "SocialNumber", ClaimValue = "Invalid value" }
        ]));
