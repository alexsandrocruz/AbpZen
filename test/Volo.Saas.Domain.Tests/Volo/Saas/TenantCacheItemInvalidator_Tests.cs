...
public class TenantConfigurationCacheItemInvalidator_Tests : SaasDomainTestBase
{
    ...
    public async Task Get_Tenant_Should_Cached()
    {
        ...
        (await _cache.GetAsync(TenantConfigurationCacheItem.CalculateCacheKey(volosoft.Id, null))).ShouldBeNull();
        (await _cache.GetAsync(TenantConfigurationCacheItem.CalculateCacheKey(null, volosoft.NormalizedName))).ShouldBeNull();

        await _tenantStore.FindAsync(volosoft.Id);
        (await _cache.GetAsync(TenantConfigurationCacheItem.CalculateCacheKey(volosoft.Id, null))).ShouldNotBeNull();

        await _tenantStore.FindAsync(volosoft.NormalizedName);
        (await _cache.GetAsync(TenantConfigurationCacheItem.CalculateCacheKey(null, volosoft.NormalizedName))).ShouldNotBeNull();
    }
    ...
    public async Task Cache_Should_Invalidator_When_Tenant_Changed()
    {
        ...
        await _tenantManager.ChangeNameAsync(abp, _tenantNormalizer.NormalizeName("abp2"));
